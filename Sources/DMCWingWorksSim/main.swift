...
func main() {
    ...
    let airfoil = AirFoil(
        x: 0.4 * worldWidth, y: 0.4 * worldHeight, width: worldWidth / 3.0,
        alphaRad: alphaRad)
    ...
    let world = World(
        airfoil: airfoil, width: worldWidth, height: worldHeight,
        maxParticleSpeed: scaledMaxPartSpeed, windSpeed: scaledWindSpeed)
    ...
    guard
        let writer = try? WorldWriter(
            ...
            width: movieWidth, height: movieHeight, title: title)
    else {
    ...
    for sec in 1...seconds {
        for iframe in 1...fps {
            ...
            print(
                "\(sec).\(iframe)/\(seconds).\(fps): net mv \(world.netMomentum()); dt = \(dt) seconds"
            )
            t0 = tf
        }
    }
